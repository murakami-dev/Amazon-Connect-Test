...
function buildSuccessfulResponse(result) {
    const response = {
        statusCode: 200,
        headers: {
            "Access-Control-Allow-Origin": "http://" + process.env.CF_DISTRIBUTION, //←元々は"https://" + process.env.CF_DISTRIBUTION, (d2vohln8s1ojn.cloudfront.net)
...
function buildResponseFailed(err) {
    const response = {
        statusCode: 500,
        headers: {
            "Access-Control-Allow-Origin": "http://" + process.env.CF_DISTRIBUTION, //←元々は"https://" + process.env.CF_DISTRIBUTION,
            'Content-Type': 'application/json',
            'Access-Control-Allow-Credentials' : true,
            'Access-Control-Allow-Headers':'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token'
        },
